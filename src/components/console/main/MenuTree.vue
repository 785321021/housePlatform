        ...
        <Row>
            ...
            <Col :xs="24" :sm="16">
                <Form ref="menuData" :model="menuData" :rules="menuValidate" action="javascript:void(0)">
                    ...
                    <FormItem label="显示状态">
                        <i-switch v-model="menuData.bigdisplay">
        ...
        methods: {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.menuData.id == 0) {
                            var creaData = this.menuData;
                            creaData.display = creaData.bigdisplay ? 1 : 0;
                            this.$api.menu.create(creaData).then(res=>{
                                if (res.code == 200) {
                                    var creaDataEnd = res.data;
                                    creaDataEnd.bigdisplay = creaDataEnd.display == 1 ? true : false;
                                    this.menuData = creaDataEnd;
                                    // 更新左侧的树
                                    this.getMenuTree();
                        ...
                        {
                            var editData = this.menuData;
                             console.log(this.menuData)
                            editData.display = editData.bigdisplay == true ? 1: 0;
                            this.$api.menu.edit(editData).then(
                                res=>{
                                if (res.code == 200) {
                                    // console.log(res.data);
                                    var editDataEnd = res.data;
                                    editDataEnd.bigdisplay = editDataEnd.display == 1 ? true : false;
                                    this.menuData = editDataEnd;
                                    // 更新左侧的树
                                    this.getMenuTree();
            ...
            getMenuTree:function(){
                ...
                this.$api.menu.tree().then(res=>{
                    ...
                    getData.bigdisplay = getData.display == 1 ? true : false;
                  this.menutree = getData;
                });
            },
            ...
            detail (data) {
                ...
                this.$api.menu.detail({'id':data.menu_id}).then(res=>{
                    if (res.code == 200) {
                        var postData = res.data;
                        postData.bigdisplay = postData.display == 1 ? true : false;
                        this.menuData = postData;
            ...
            remove (root, node, data) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        this.$api.menu.remove({'id':data.menu_id}).then(res=>{
                            if(res.code == 200)
                            {
                                // 更新左侧的树
                                this.getMenuTree();
